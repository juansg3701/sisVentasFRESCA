...
<?php
	...
	$query="SELECT p.id_producto, p.plu, p.ean, p.nombre, c.nombre as categoria_id_categoria, p.unidad_de_medida, i.valor_impuesto as impuestos_id_impuestos, d.valor_descuento as descuento_id_descuento, p.stock_minimo, p.necesita_peso, p.imagen, p.precio_1, p.precio_2, p.precio_3, p.precio_4, p.costo_compra, pv.no as punto_venta_id_punto_venta, p.empleado_id_empleado, p.fecha_registro  FROM producto as p, categoria_productos as c, impuestos as i, descuento as d, punto_venta as pv WHERE p.categoria_id_categoria=c.id_categoria and p.impuestos_id_impuestos=i.id_impuestos and p.descuento_id_descuento=d.id_descuento and p.punto_venta_id_punto_venta=pv.id_punto_venta ORDER BY id_producto DESC";
