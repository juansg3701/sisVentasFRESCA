...
<body>
	<div class="row" align="center">
			<div align="center"><h3>REPORTES DE BANCOS</h3></div><br>
			@if (count($errors)>0)
	...
	<div id=formulario>
	{!!Form::open(array('url'=>'almacen/reportes/bancos/bancos','method'=>'POST','autocomplete'=>'off'))!!}
    {{Form::token()}}
		<div id=formulario>
...
<div class="row">
				<div class="table-responsive">
					<table class="table table-striped table-bordered table-condensed table-hover">
						...
						<tr>
							<td>{{ $rep->id_rbancos}}</td>
							<td>{{ $rep->fechaActual}}</td>
							<td>{{ $rep->fechaInicial}}</td>
							<td>{{ $rep->fechaFinal}}</td>
			
							<td>
								<a href="{{URL::action('reportesBancos@show',$rep->id_rbancos)}}"><button class="btn btn-info">Gráfica</button></a>
								<a href="{{URL::action('reportesBancosDescargas@edit',$rep->id_rbancos)}}"><button class="btn btn-warning"><i>pdf</i></button></a>
								<a href="{{URL::action('reportesBancosDescargas@show',$rep->id_rbancos)}}"><button class="btn btn-success"><i>xls</i></button></a>
								<a href="" data-target="#modal-delete-{{$rep->id_rbancos}}" data-toggle="modal"><button class="btn btn-danger">Eliminar</button></a>
							</td>
						</tr>
						@include('almacen.reportes.bancos.bancos.modal')	
						@endforeach
					</table>
				</div>
